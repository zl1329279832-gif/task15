var Renderer=(function(){
  ...
  var animTime=0,lastFrameTime=0,animFrameId=null,running=false,paused=false;
  var perf={frameCount:0,fps:0,lastFpsTime:0};
  var hitAreas=[];

  /* ---- 帧快照同步 ---- */
  var pendingSnapshot=null;
  var appliedFrameId=0;
  ...
  function start(){if(running)return;running=true;paused=false;lastFrameTime=performance.now();perf.lastFpsTime=lastFrameTime;perf.frameCount=0;animFrameId=requestAnimationFrame(_loop);}
  function stop(){running=false;paused=false;if(animFrameId){cancelAnimationFrame(animFrameId);animFrameId=null;}}
  function pause(){if(!running||paused)return;paused=true;if(animFrameId){cancelAnimationFrame(animFrameId);animFrameId=null;}}
  function resume(){if(!running||!paused)return;paused=false;lastFrameTime=performance.now();animFrameId=requestAnimationFrame(_loop);}
  function destroy(){stop();pendingSnapshot=null;appliedFrameId=0;currentEquipment=null;currentPipes=null;tankConfig=null;hitAreas=[];ctx=null;canvas=null;}

  /* ---- 快照应用 ---- */
  function applySnapshot(snap){
    if(!snap)return;
    pendingSnapshot=snap;
    /* 如果暂停中，立刻提交并重绘一帧，保证画面与数据同步 */
    if(paused&&running){
      _commitSnapshot();
      _renderOneFrame();
    }
  ...
  function _commitSnapshot(){
    ...
    appliedFrameId=snap.frameId;

    /* 原子化更新所有渲染状态 */
    for(var i=0;i<currentEquipment.length&&i<snap.equipment.length;i++){
      currentEquipment[i].state=snap.equipment[i].state;
    }
    for(var j=0;j<currentPipes.length&&j<snap.pipes.length;j++){
      currentPipes[j].active=snap.pipes[j].active;
    }
    if(tankConfig) tankConfig.level=snap.tankLevel;
    isNight=!!snap.isNight;
  }

  function _renderOneFrame(){
    if(!ctx)return;
  ...
  function _loop(now){
    ...
    /* 帧边界：原子化提交待处理快照 */
    if(pendingSnapshot) _commitSnapshot();
  ...
  function getAppliedFrameId(){return appliedFrameId;}
  ...
  return{
    init:init,resize:resize,start:start,stop:stop,pause:pause,resume:resume,destroy:destroy,
    applySnapshot:applySnapshot,
    hitTest:hitTest,rebuildHitAreas:rebuildHitAreas,
    setNight:setNight,setSelected:setSelected,setHover:setHover,
    setTankLevel:setTankLevel,setActivePipes:setActivePipes,
    getFPS:getFPS,getAppliedFrameId:getAppliedFrameId,isPaused:isPaused
  };
})();
